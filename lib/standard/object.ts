...
export function isKeyEqual(valueKeys: Array<string>, compareKeys: Array<string>) {
  // key order is not a matter
  return compareKeys.every(function(index: string) {
    return valueKeys.indexOf(index) !== -1;
  });
}

// Referece: https://stackoverflow.com/a/16788517/139361
export function isEqual(value: any, compare: any): boolean {
  ...
  return (
    ...
    valueKeys.every(function(index: string) {
      return isEqual(value[index], compare[index]);
    })
  );
}
